...
FROM (
    SELECT schools.id AS school_id , schools.name AS school_name , students.id AS student_id , students.first_name AS student_first_name , students.last_name AS student_last_name , AVG(scores.score) AS avg_score , RANK()
        OVER(PARTITION BY schools.id  ORDER BY AVG(scores.score) DESC
    ) AS rnk
    FROM RoyHadad_Schools schools
